...
# -----------------------------------------------------------------------------
# HIDROSED_V1 · funciones adicionales de reporte morfométrico
# No modifican la delimitación original de Foja Cero v3; solo calculan métricas
# a partir de las cuencas ya delimitadas.
# -----------------------------------------------------------------------------
def hidrosed_morphometry_row(b: BasinResult, dem: np.ndarray, transform: Affine, valid: np.ndarray) -> Dict[str, object]:
    area = float(b.area_km2)
    per = float(b.perimeter_km)
    kc = 0.28 * per / math.sqrt(area) if area > 0 else float("nan")
    eq_diam = 2.0 * math.sqrt(area / math.pi) if area > 0 else float("nan")
    minx, miny, maxx, maxy = b.polygon_utm.bounds
    length_km = max(maxx - minx, maxy - miny) / 1000.0
    width_km = min(maxx - minx, maxy - miny) / 1000.0
    ff = area / (length_km ** 2) if length_km > 0 else float("nan")
    elong = eq_diam / length_km if length_km > 0 else float("nan")
    zmin = zmean = zmax = relief = slope_mean = float("nan")
    try:
        mask = geometry_mask([mapping(b.polygon_utm)], out_shape=dem.shape, transform=transform, invert=True)
        vals = dem[mask & valid & np.isfinite(dem)]
        if vals.size:
            zmin = float(np.nanmin(vals))
            zmean = float(np.nanmean(vals))
            zmax = float(np.nanmax(vals))
            relief = zmax - zmin
            cell_x = abs(float(transform.a)) if transform.a else 1.0
            cell_y = abs(float(transform.e)) if transform.e else cell_x
            gy, gx = np.gradient(np.where(np.isfinite(dem), dem, np.nan), cell_y, cell_x)
            sm = np.sqrt(gx * gx + gy * gy) * 100.0
            svals = sm[mask & valid & np.isfinite(sm)]
            if svals.size:
                slope_mean = float(np.nanmean(svals))
    except Exception:
        pass
    return {
        "Elemento": b.name,
        "Área km²": round(area, 3),
        "Perímetro km": round(per, 3),
        "Coef. compacidad Kc": round(kc, 3) if math.isfinite(kc) else None,
        "Diámetro equivalente km": round(eq_diam, 3) if math.isfinite(eq_diam) else None,
        "Longitud envolvente km": round(length_km, 3),
        "Ancho envolvente km": round(width_km, 3),
        "Factor de forma": round(ff, 4) if math.isfinite(ff) else None,
        "Relación elongación": round(elong, 4) if math.isfinite(elong) else None,
        "Cota mín m": round(zmin, 2) if math.isfinite(zmin) else None,
        "Cota media m": round(zmean, 2) if math.isfinite(zmean) else None,
        "Cota máx m": round(zmax, 2) if math.isfinite(zmax) else None,
        "Desnivel m": round(relief, 2) if math.isfinite(relief) else None,
        "Pendiente media DEM %": round(slope_mean, 2) if math.isfinite(slope_mean) else None,
        "Confianza preliminar %": round(float(b.confidence), 1),
        "Toca borde DEM": bool(b.touches_dem_edge),
    }


def hidrosed_plot_basin_image(basin_support: BasinResult, basin_hydro: BasinResult, inter_utm, corridor_detail_utm, axis_utm, axis_segment_utm, pc_cuenca_utm, pc_hidro_utm):
    fig, ax = plt.subplots(figsize=(8, 7))
    def _plot_geom(geom, label, alpha=0.22, lw=1.8):
        if geom is None or geom.is_empty:
            return
        geoms = [geom] if geom.geom_type in ["Polygon", "LineString"] else list(getattr(geom, "geoms", []))
        first = True
        for g in geoms:
            if g.geom_type == "Polygon":
                x, y = g.exterior.xy
                ax.fill(x, y, alpha=alpha, linewidth=lw, fill=True, label=label if first else None)
                ax.plot(x, y, linewidth=lw)
            elif g.geom_type == "LineString":
                x, y = g.xy
                ax.plot(x, y, linewidth=lw, label=label if first else None)
            first = False
    _plot_geom(basin_support.polygon_utm, "Cuenca soporte", 0.18, 2.0)
    _plot_geom(basin_hydro.polygon_utm, "Subcuenca hidrológica", 0.20, 2.0)
    _plot_geom(inter_utm, "Intersección", 0.18, 1.4)
    _plot_geom(corridor_detail_utm, "Corredor curvas/eje", 0.14, 1.0)
    _plot_geom(axis_utm, "Eje completo", 0.0, 1.4)
    _plot_geom(axis_segment_utm, "Tramo útil", 0.0, 3.0)
    ax.scatter([pc_cuenca_utm.x, pc_hidro_utm.x], [pc_cuenca_utm.y, pc_hidro_utm.y], s=35, marker="o", label="PCs")
    ax.set_title("Cuencas delimitadas y tramo útil del eje")
    ...
    with tab1:
        ...
        try:
            ...
            morph_df = pd.DataFrame([
                hidrosed_morphometry_row(basin_support, dem, dem_transform, valid),
                hidrosed_morphometry_row(basin_hydro, dem, dem_transform, valid),
            ])
            st.write("**Propiedades morfométricas de cuenca**")
            st.dataframe(morph_df, use_container_width=True)

            with st.expander("Imagen de cuenca delimitada y tramo útil", expanded=True):
                fig_basin = hidrosed_plot_basin_image(
                    basin_support, basin_hydro, inter_utm, corridor_detail_utm,
                    axis_utm, axis_segment_utm, pc_cuenca_utm, pc_hidro_utm
                )
                st.pyplot(fig_basin)
            ...
            summary = {
                ...
                "morfometria": morph_df.to_dict(orient="records"),
            ...
            st.session_state["foja_cero_v3_summary"] = summary
